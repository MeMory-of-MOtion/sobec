...
DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::
...
template <typename Scalar>
DifferentialActionModelSoftContact3DFwdDynamicsTpl<
    Scalar>::~DifferentialActionModelSoftContact3DFwdDynamicsTpl() {}
...
void DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::calc(
  ...
  if (active_contact_) {
    ...
    d->lv = pinocchio::getFrameVelocity(this->get_pinocchio(), d->pinocchio,
                ...
                .linear();
    d->f = -Kp_ * d->oRf.transpose() *
               (d->pinocchio.oMf[frameId_].translation() - oPc_) -
           Kv_ * d->lv;
    ...
    if (ref_ != pinocchio::LOCAL) {
      d->f = -Kp_ * (d->pinocchio.oMf[frameId_].translation() - oPc_) -
             Kv_ * d->oRf * d->lv;
      d->pinForce = pinocchio::ForceTpl<Scalar>(d->oRf.transpose() * d->f,
                                                Vector3s::Zero());
      d->fext[parentId_] = jMf_.act(d->pinForce);
    }
    ...
    //     v); d->pinocchio.M.diagonal() += this->get_armature();
    //     pinocchio::cholesky::decompose(this->get_pinocchio(), d->pinocchio);
    //     d->Minv.setZero();
    //     pinocchio::cholesky::computeMinv(this->get_pinocchio(), d->pinocchio,
    //     d->Minv); d->u_drift = d->multibody.actuation->tau -
    //     d->pinocchio.nle; d->xout.noalias() = d->Minv * d->u_drift;
    // }
  }
...
void DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::calcDiff(
  ...
  if (active_contact_) {
    ...
    pinocchio::getFrameVelocityDerivatives(this->get_pinocchio(), d->pinocchio,
                                           frameId_, pinocchio::LOCAL,
                                           d->lv_partial_dq, d->lv_partial_dv);
    d->df_dx.leftCols(nv) =
        -Kp_ * (d->lJ.topRows(3) +
                pinocchio::skew(
                    d->oRf.transpose() *
                    (d->pinocchio.oMf[frameId_].translation() - oPc_)) *
                    d->lJ.bottomRows(3)) -
        Kv_ * d->lv_partial_dq.topRows(3);
    d->df_dx.rightCols(nv) = -Kv_ * d->lv_partial_dv.topRows(3);
    // copy for later
    d->df_dx_copy = d->df_dx;
    // rotate force derivatives if not local
    if (ref_ != pinocchio::LOCAL) {
      d->df_dx.leftCols(nv) =
          d->oRf * d->df_dx_copy.leftCols(nv) -
          pinocchio::skew(d->oRf * d->f_copy) * d->oJ.bottomRows(3);
      d->df_dx.rightCols(nv) = d->oRf * d->df_dx_copy.rightCols(nv);
    }
    // Compute ABA derivatives (same in LOCAL and LWA for 3D contact)
    pinocchio::computeABADerivatives(this->get_pinocchio(), d->pinocchio, q, v,
                                     d->multibody.actuation->tau, d->fext_copy,
                                     d->aba_dq, d->aba_dv, d->aba_dtau);
    d->Fx.leftCols(nv) = d->aba_dq + d->aba_dtau *
                                         d->lJ.topRows(3).transpose() *
                                         d->df_dx_copy.leftCols(nv);
    d->Fx.rightCols(nv) = d->aba_dv + d->aba_dtau *
                                          d->lJ.topRows(3).transpose() *
                                          d->df_dx_copy.rightCols(nv);
...
template <typename Scalar>
void DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::set_force_cost(
...
template <typename Scalar>
void DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::set_Kp(
    const Scalar inKp) {
  if (inKp < 0.) {
    throw_pretty("Invalid argument: "
...
template <typename Scalar>
void DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::set_Kv(
    const Scalar inKv) {
  if (inKv < 0.) {
    throw_pretty("Invalid argument: "
...
template <typename Scalar>
void DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::set_oPc(
...
template <typename Scalar>
void DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::set_force_des(
...
template <typename Scalar>
void DifferentialActionModelSoftContact3DFwdDynamicsTpl<
    Scalar>::set_force_weight(const Scalar inForceWeight) {
  if (inForceWeight < 0.) {
    throw_pretty("Invalid argument: "
...
template <typename Scalar>
void DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::set_ref(
    const pinocchio::ReferenceFrame inRef) {
  ref_ = inRef;
}

template <typename Scalar>
void DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::set_id(
...
template <typename Scalar>
const Scalar
DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::get_Kp() const {
  return Kp_;
}

template <typename Scalar>
const Scalar
DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::get_Kv() const {
  return Kv_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::Vector3s&
DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::get_oPc() const {
  return oPc_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::Vector3s&
DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::get_force_des()
    const {
  return force_des_;
}

template <typename Scalar>
const Scalar DifferentialActionModelSoftContact3DFwdDynamicsTpl<
    Scalar>::get_force_weight() const {
  return force_weight_;
}

template <typename Scalar>
const pinocchio::ReferenceFrame&
DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::get_ref() const {
  return ref_;
}

template <typename Scalar>
const pinocchio::FrameIndex&
DifferentialActionModelSoftContact3DFwdDynamicsTpl<Scalar>::get_id() const {
  return frameId_;
}
