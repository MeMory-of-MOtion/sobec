...
DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::
    DAMSoftContact3DAugmentedFwdDynamicsTpl(
        ...
        const Vector3s& oPc, const pinocchio::ReferenceFrame ref)
  ...
  Base::set_u_lb(Scalar(-1.) *
                 this->get_pinocchio().effortLimit.tail(this->get_nu()));
  Base::set_u_ub(Scalar(+1.) *
                 this->get_pinocchio().effortLimit.tail(this->get_nu()));
...
template <typename Scalar>
DAMSoftContact3DAugmentedFwdDynamicsTpl<
    Scalar>::~DAMSoftContact3DAugmentedFwdDynamicsTpl() {}
...
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::calc(
  ...
  if (active_contact_) {
    ...
    if (ref_ != pinocchio::LOCAL) {
      d->pinForce =
          pinocchio::ForceTpl<Scalar>(d->oRf.transpose() * f, Vector3s::Zero());
      d->fext[parentId_] = jMf_.act(d->pinForce);
    }
    ...
    if (with_armature_) {
      ...
      d->Minv.setZero();
      pinocchio::cholesky::computeMinv(this->get_pinocchio(), d->pinocchio,
                                       d->Minv);
      d->u_drift = d->multibody.actuation->tau - d->pinocchio.nle;
      //  Compute jacobian transpose lambda
      pinocchio::getFrameJacobian(this->get_pinocchio(), d->pinocchio, frameId_,
                                  pinocchio::LOCAL, d->lJ);
      d->xout.noalias() =
          d->Minv * d->u_drift + d->Minv * d->lJ.topRows(3).transpose() *
                                     d->pinForce.linear();  // d->JtF;
      // ABA without armature
    } else {
      d->xout = pinocchio::aba(this->get_pinocchio(), d->pinocchio, q, v,
                               d->multibody.actuation->tau, d->fext);
    }
    // Compute time derivative of contact force : need to forward kin with
    // current acc
    pinocchio::forwardKinematics(this->get_pinocchio(), d->pinocchio, q, v,
                                 d->xout);
    d->la = pinocchio::getFrameAcceleration(this->get_pinocchio(), d->pinocchio,
                                            frameId_, pinocchio::LOCAL)
                .linear();
    d->lv = pinocchio::getFrameVelocity(this->get_pinocchio(), d->pinocchio,
                                        frameId_, pinocchio::LOCAL)
                .linear();
    ...
    // Rotate if not f not in LOCAL
    if (ref_ != pinocchio::LOCAL) {
      d->oa = pinocchio::getFrameAcceleration(this->get_pinocchio(),
                                              d->pinocchio, frameId_,
                                              pinocchio::LOCAL_WORLD_ALIGNED)
                  .linear();
      d->ov =
          pinocchio::getFrameVelocity(this->get_pinocchio(), d->pinocchio,
                                      frameId_, pinocchio::LOCAL_WORLD_ALIGNED)
              .linear();
...
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::calcDiff(
  ...
  if (active_contact_) {
    // Compute Jacobian
    // pinocchio::framesForwardKinematics(this->get_pinocchio(), d->pinocchio,
    // q);
    pinocchio::getFrameJacobian(this->get_pinocchio(), d->pinocchio, frameId_,
                                pinocchio::LOCAL, d->lJ);
    ...
    // No armature
    if (!with_armature_) {
      pinocchio::computeABADerivatives(this->get_pinocchio(), d->pinocchio, q,
                                       v, d->multibody.actuation->tau, d->fext,
      ...
      d->aba_df = d->aba_dtau * d->lJ.topRows(3).transpose() * jMf_.rotation() *
                  Matrix3s::Identity();
      // Skew term added to RNEA derivatives when force is expressed in LWA
      if (ref_ != pinocchio::LOCAL) {
        d->Fx.leftCols(nv) += d->aba_dtau * d->lJ.topRows(3).transpose() *
                              pinocchio::skew(d->oRf.transpose() * f) *
                              d->lJ.bottomRows(3);
        // Rotate dABA/df
      ...
      pinocchio::computeRNEADerivatives(this->get_pinocchio(), d->pinocchio, q,
                                        v, d->xout, d->fext);
      ...
      // Compute derivatives of d->xout (ABA) w.r.t. f in LOCAL
      d->aba_df = d->Minv * d->lJ.topRows(3).transpose() * jMf_.rotation() *
                  Matrix3s::Identity();
      // Skew term added to RNEA derivatives when force is expressed in LWA
      if (ref_ != pinocchio::LOCAL) {
        d->Fx.leftCols(nv) += d->Minv * d->lJ.topRows(3).transpose() *
                              pinocchio::skew(d->oRf.transpose() * f) *
                              d->lJ.bottomRows(3);
        // Rotate dABA/df
        d->aba_df = d->aba_df * d->oRf.transpose();
    ...
    // Derivatives of d->fout in LOCAL : important >> UPDATE FORWARD KINEMATICS
    // with d->xout
    pinocchio::getFrameVelocityDerivatives(this->get_pinocchio(), d->pinocchio,
                                           frameId_, pinocchio::LOCAL, d->lv_dq,
                                           d->lv_dv);
    ...
    pinocchio::getFrameAccelerationDerivatives(
        this->get_pinocchio(), d->pinocchio, frameId_, pinocchio::LOCAL,
        d->v_dv, d->a_dq, d->a_dv, d->a_da);
    d->da_dx.topRows(3).leftCols(nv) =
        d->a_dq.topRows(3) + d->a_da.topRows(3) * d->Fx.leftCols(nv);
    d->da_dx.topRows(3).rightCols(nv) =
        d->a_dv.topRows(3) + d->a_da.topRows(3) * d->Fx.rightCols(nv);
    ...
    if (ref_ != pinocchio::LOCAL) {
      pinocchio::getFrameJacobian(this->get_pinocchio(), d->pinocchio, frameId_,
                                  pinocchio::LOCAL_WORLD_ALIGNED, d->oJ);
      d->dfdt_dx.leftCols(nv) =
          d->oRf * d->dfdt_dx_copy.leftCols(nv) -
          pinocchio::skew(d->oRf * d->fout_copy) * d->oJ.bottomRows(3);
...
template <typename Scalar>
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::set_force_cost(
    const Vector3s& force_des, const Scalar force_weight) {
  if (force_weight < 0.) {
    throw_pretty("Invalid argument: "
...
template <typename Scalar>
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::set_Kp(
    const Scalar inKp) {
  if (inKp < 0.) {
    throw_pretty("Invalid argument: "
...
template <typename Scalar>
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::set_Kv(
    const Scalar inKv) {
  if (inKv < 0.) {
    throw_pretty("Invalid argument: "
...
template <typename Scalar>
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::set_oPc(
    const Vector3s& inoPc) {
  if (inoPc.size() != 3) {
    throw_pretty("Invalid argument: "
...
template <typename Scalar>
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::set_force_des(
    const Vector3s& inForceDes) {
...
template <typename Scalar>
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::set_force_weight(
...
template <typename Scalar>
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::set_ref(
    const pinocchio::ReferenceFrame inRef) {
  ref_ = inRef;
}

template <typename Scalar>
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::set_id(
    const pinocchio::FrameIndex inId) {
  frameId_ = inId;
}
...
DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::get_oPc() const {
  return oPc_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::Vector3s&
DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::get_force_des() const {
  return force_des_;
}

template <typename Scalar>
const Scalar DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::get_force_weight()
    const {
...
DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::get_ref() const {
  return ref_;
}

template <typename Scalar>
const pinocchio::FrameIndex&
DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::get_id() const {
  return frameId_;
}
...
DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::get_armature() const {
  return armature_;
}

template <typename Scalar>
void DAMSoftContact3DAugmentedFwdDynamicsTpl<Scalar>::set_armature(
    const VectorXs& armature) {
  if (static_cast<std::size_t>(armature.size()) !=
      this->get_state()->get_nv()) {
    throw_pretty("Invalid argument: "
                 << "The armature dimension is wrong (it should be " +
                        std::to_string(this->get_state()->get_nv()) + ")");
  }
  armature_ = armature;
  with_armature_ = true;
}
