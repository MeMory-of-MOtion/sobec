...
template <typename Scalar>
void ResidualModelFlyAngleTpl<Scalar>::calc(
  ...
  data->r = (d->rotation_alpha * pinocchio::getFrameVelocity(pin_model_, *d->pinocchio, frame_id,
...
template <typename Scalar>
void ResidualModelFlyAngleTpl<Scalar>::calcDiff(
    const boost::shared_ptr<ResidualDataAbstract>& data,
    const Eigen::Ref<const VectorXs>& /*x*/,
    const Eigen::Ref<const VectorXs>&) {
  Data* d = static_cast<Data*>(data.get());
  ...
  data->Rx.leftCols(nv).row(0) += (d->rotation_alpha_dt * pinocchio::getFrameVelocity(pin_model_, *d->pinocchio, frame_id,
                                        pinocchio::LOCAL_WORLD_ALIGNED).linear())[0] * d->ez * d->alpha_dt * d->o_dv_dv.row(0);
  data->Rx.leftCols(nv).row(1) += (d->rotation_alpha_dt * pinocchio::getFrameVelocity(pin_model_, *d->pinocchio, frame_id,
                                        pinocchio::LOCAL_WORLD_ALIGNED).linear())[1] * d->ez * d->alpha_dt * d->o_dv_dv.row(0);
}

template <typename Scalar>
boost::shared_ptr<ResidualDataAbstractTpl<Scalar> >
ResidualModelFlyAngleTpl<Scalar>::createData(DataCollectorAbstract* const data) {
...
ResidualModelFlyAngleTpl<Scalar>::get_frame_id() const {
  return frame_id;
}

template <typename Scalar>
void ResidualModelFlyAngleTpl<Scalar>::set_frame_id(
