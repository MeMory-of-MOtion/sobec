...
void ResidualModelFlyAngleTpl<Scalar>::calc(
  ...
  d->sig =
      1 /
      (1 + exp(-width * (d->pinocchio->oMf[frame_id].translation()[0] - dist)));
  ...
  d->ez = exp(-slope *
              (d->pinocchio->oMf[frame_id].translation()[2] - height * d->sig));

  data->r = (d->rotation_alpha *
             pinocchio::getFrameVelocity(pin_model_, *d->pinocchio, frame_id,
...
void ResidualModelFlyAngleTpl<Scalar>::calcDiff(
  ...
  d->alpha_dt =
      height * d->sig_ddt / (1 + (height * d->sig_dt) * (height * d->sig_dt));
  ...
  data->Rx.leftCols(nv) =
      (d->rotation_alpha * d->o_dv_dq).template topRows<2>();
  data->Rx.rightCols(nv) =
      (d->rotation_alpha * d->o_dv_dv).template topRows<2>();
  ...
  data->Rx.leftCols(nv).row(0) -=
      data->r[0] * slope *
      (d->o_dv_dv.row(2) - height * d->sig_dt * d->o_dv_dv.row(0));
...
ResidualModelFlyAngleTpl<Scalar>::createData(
