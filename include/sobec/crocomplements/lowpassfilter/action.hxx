...
IntegratedActionModelLPFTpl<Scalar>::IntegratedActionModelLPFTpl(
    ...
    const Scalar& time_step, const bool& with_cost_residual, const Scalar& fc,
    const bool& tau_plus_integration, const int& filter,
    const bool& is_terminal)
    : Base(model->get_state(), model->get_nu(),
           model->get_nr() + 2 * model->get_nu()),
      differential_(model),
  ...
  activation_model_w_lim_ = boost::make_shared<ActivationModelQuadraticBarrier>(
...
void IntegratedActionModelLPFTpl<Scalar>::calc(
  ...
  const std::size_t& ny =
      boost::static_pointer_cast<StateLPF>(state_)->get_ny();
  const std::size_t& nu = differential_->get_nu();
  const std::size_t& nw =
      boost::static_pointer_cast<StateLPF>(state_)->get_nw();
  ...
  if (static_cast<std::size_t>(d->Fw.cols()) !=
      boost::static_pointer_cast<StateLPF>(state_)->get_nw()) {
    throw_pretty(
        "Invalid argument: "
  ...
  if (static_cast<std::size_t>(d->Lw.size()) !=
      boost::static_pointer_cast<StateLPF>(state_)->get_nw()) {
    throw_pretty(
        "Invalid argument: "
  ...
  if (!is_terminal_) {
    ...
    d->dy.tail(nu).noalias() =
        (1 - alpha_) * (w - tau);  // get dtau_q(w, tau, alpha)
    state_->integrate(
        y, d->dy,
    ...
    if (wreg_ != 0) {
      res_w_reg = w - wref_;
      d->cost += Scalar(0.5 * time_step_ * wreg_ * res_w_reg.transpose() *
                        res_w_reg);  // w reg
    }
    if (wlim_ != 0) {
      activation_model_w_lim_->calc(
          d->activation, w);  // Compute limit cost torque residual of w
      res_w_lim = w;
      d->cost +=
          Scalar(0.5 * time_step_ * wlim_ * d->activation->a_value);  // w lim
    }
  }  // running model
...
void IntegratedActionModelLPFTpl<Scalar>::calcDiff(
  ...
  if (!tau_plus_integration_) {
    ...
    if (!is_terminal_) {
      ...
      d->Lyy.block(0, ndx, ndx, nu).noalias() =
          time_step_ * d->differential->Lxu;
      d->Lyy.block(ndx, 0, nu, ndx).noalias() =
          time_step_ * d->differential->Lxu.transpose();
      d->Lyy.bottomRightCorner(nu, nu).noalias() =
          time_step_ * d->differential->Luu;
      // Partials of hard-coded cost+(wreg) & cost+(wlim) w.r.t. (y,w)
      if (wreg_ != 0) {
        d->Lw.noalias() = time_step_ * wreg_ *
                          d->r.segment(differential_->get_nr(), nw);  // w reg
        d->Lww.diagonal().array() = Scalar(time_step_ * wreg_);       // w reg
      }  // wreg !=0
      if (wlim_ != 0) {
        d->Lw.noalias() += time_step_ * wlim_ * d->activation->Ar;  // w lim
        d->Lww.diagonal() +=
            time_step_ * wlim_ * d->activation->Arr.diagonal();  // w lim
      }                                                          // wlim !=0
  ...
  // TAU PLUS INTEGRATION
    ...
    if (enable_integration_) {
      ...
      d->Fy.block(0, 0, nv, ndx).noalias() = da_dx * time_step2_;
      d->Fy.block(nv, 0, nv, ndx).noalias() = da_dx * time_step_;
      d->Fy.block(0, ndx, nv, nu).noalias() =
          alpha_ * alpha_ * da_du * time_step2_;
      ...
      d->Lyy.topLeftCorner(ndx, ndx).noalias() =
          time_step_ * d->differential->Lxx;
      d->Lyy.block(0, ndx, ndx, nu).noalias() =
          alpha_ * time_step_ * d->differential->Lxu;
      d->Lyy.block(ndx, 0, nu, ndx).noalias() =
          alpha_ * time_step_ * d->differential->Lxu.transpose();
      d->Lyy.bottomRightCorner(nu, nu).noalias() =
          alpha_ * alpha_ * time_step_ * d->differential->Luu;
      d->Lyw.topRows(ndx).noalias() =
          (1 - alpha_) * time_step_ * d->differential->Lxu;
      d->Lyw.bottomRows(nu).noalias() =
          (1 - alpha_) * alpha_ * time_step_ * d->differential->Luu;
      d->Lww.noalias() =
      ...
      if (!is_terminal_) {
        ...
        d->Lww.diagonal() +=
            time_step_ * wlim_ * d->activation->Arr.diagonal();  // lim
      }
      ...
      d->Lyy.block(ndx, 0, nu, ndx).noalias() =
          alpha_ * d->differential->Lxu.transpose();
      d->Lyy.bottomRightCorner(nu, nu).noalias() =
          alpha_ * alpha_ * d->differential->Luu;
      d->Lyw.topRows(ndx).noalias() = (1 - alpha_) * d->differential->Lxu;
      d->Lyw.bottomRows(nu).noalias() =
          (1 - alpha_) * alpha_ * d->differential->Luu;
      d->Lww.noalias() = (1 - alpha_) * (1 - alpha_) * d->differential->Luu;
      // Add partials related to unfiltered torque costs w_reg, w_lim (only for
      // running models)
      if (!is_terminal_) {
        d->Lw.noalias() +=
            wreg_ * d->r.segment(differential_->get_nr(), nw);       // reg
