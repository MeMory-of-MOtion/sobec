...
StateLPFTpl<Scalar>::StateLPFTpl(boost::shared_ptr<pinocchio::ModelTpl<Scalar> > model, std::vector<int> lpf_joint_ids)
  ...
  for (std::size_t i = 0; i < lpf_joint_ids.size(); i++) {
    if ((int)model->nvs[lpf_joint_ids[i]] != (int)1) {
      throw_pretty("Invalid argument: "
                   << "Joint " << lpf_joint_ids[i] << " has nv=" << model->nvs[lpf_joint_ids[i]]
