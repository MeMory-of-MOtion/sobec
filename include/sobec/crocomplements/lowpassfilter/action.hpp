...
class IntegratedActionModelLPFTpl : public ActionModelAbstractTpl<_Scalar> {
 public:
  ...
  IntegratedActionModelLPFTpl(boost::shared_ptr<DifferentialActionModelAbstract> model,
                              std::vector<std::string> lpf_joint_names = {}, const Scalar& time_step = Scalar(1e-3),
  ...
  const std::vector<int>& get_lpf_torque_ids() const { return lpf_torque_ids_; };
  const std::vector<int>& get_non_lpf_torque_ids() const { return non_lpf_torque_ids_; };
 ...
 public:
  boost::shared_ptr<ActivationModelQuadraticBarrier> activation_model_tauLim_;  //!< for lim cost
...
struct IntegratedActionDataLPFTpl : public ActionDataAbstractTpl<_Scalar> {
  ...
  explicit IntegratedActionDataLPFTpl(Model<Scalar>* const model) : Base(model), tau_tmp(model->get_nu()) {
    tau_tmp.setZero();
    ...
    // for wlim cost
    activation =
        boost::static_pointer_cast<ActivationDataQuadraticBarrier>(model->activation_model_tauLim_->createData());
  }
