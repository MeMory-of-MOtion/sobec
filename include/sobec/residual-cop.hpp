...
class ResidualModelCenterOfPressureTpl : public ResidualModelAbstractTpl<_Scalar> {
 public:
   ...
   * link is attached
   */
  ResidualModelCenterOfPressureTpl(boost::shared_ptr<StateMultibody> state, const pinocchio::FrameIndex contact_id,
                                   const std::size_t nu);
...
struct ResidualDataCenterOfPressureTpl : public ResidualDataAbstractTpl<_Scalar> {
  ...
  ResidualDataCenterOfPressureTpl(Model<Scalar> *const model, DataCollectorAbstract *const data) : Base(model, data) {
