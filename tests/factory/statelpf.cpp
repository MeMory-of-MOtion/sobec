  ...
  switch (lpf_mask_type) {
    case LPFJointMaskType::ALL: {
      for (std::vector<std::string>::iterator iter = model->names.begin(); iter != model->names.end(); ++iter) {
        if (static_cast<int>(model->getJointId(*iter)) < model->njoints && model->nvs[model->getJointId(*iter)] == 1) {
...
boost::shared_ptr<sobec::StateLPF> StateLPFModelFactory::create(StateLPFModelTypes::Type state_type,
                                                                LPFJointMaskType lpf_mask_type) const {
