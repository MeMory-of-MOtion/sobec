...
void test_state_dimension(StateLPFModelTypes::Type state_type) {
  ...
  BOOST_CHECK(stateLPFAll->get_ny() ==
              (stateLPFAll->get_nq() + stateLPFAll->get_nv() + stateLPFAll->get_ntau()));
  ...
  BOOST_CHECK(stateLPFRand->get_ny() ==
              (stateLPFRand->get_nq() + stateLPFRand->get_nv() + stateLPFRand->get_ntau()));
...
void test_Jdiff_num_diff_firstsecond(StateLPFModelTypes::Type state_type) {
  ...
  Eigen::MatrixXd Jdiff_num_diff_secondrand(
      Eigen::MatrixXd::Zero(stateLPFRand->get_ndy(), stateLPFRand->get_ndy()));
  state_num_diffrand.Jdiff(x1rand, x2rand, Jdiff_num_diff_firstrand, Jdiff_num_diff_tmprand,
                       crocoddyl::first);
  state_num_diffrand.Jdiff(x1rand, x2rand, Jdiff_num_diff_tmprand, Jdiff_num_diff_secondrand,
                       crocoddyl::second);
  // Computing the partial derivatives of the difference function separately
  ...
  BOOST_CHECK(
      (Jdiff_num_diff_secondrand - Jdiff_num_diff_both_secondrand).isZero(1e-9));
}
...
void test_Jint_num_diff_firstsecond(StateLPFModelTypes::Type state_type) {
  ...
  Eigen::MatrixXd Jint_num_diff_secondrand(
      Eigen::MatrixXd::Zero(stateLPFRand->get_ndy(), stateLPFRand->get_ndy()));
  state_num_diffrand.Jintegrate(xrand, dxrand, Jint_num_diff_firstrand, Jint_num_diff_tmprand,
                            crocoddyl::first);
  state_num_diffrand.Jintegrate(xrand, dxrand, Jint_num_diff_tmprand, Jint_num_diff_secondrand,
                            crocoddyl::second);
  // Computing the partial derivatives of the given function separately
...
void test_JintegrateTransport(StateLPFModelTypes::Type state_type) {
  ...
  stateLPFAll->Jintegrate(x, dx, Jint_1, Jint_2);
  Eigen::MatrixXd Jref(
      Eigen::MatrixXd::Random(stateLPFAll->get_ndy(), 2 * stateLPFAll->get_ndy()));
  ...
  Eigen::MatrixXd Jint_2rand(
      Eigen::MatrixXd::Zero(stateLPFRand->get_ndy(), stateLPFRand->get_ndy()));
  stateLPFRand->Jintegrate(xrand, dxrand, Jint_1rand, Jint_2rand);
  Eigen::MatrixXd Jrefrand(
      Eigen::MatrixXd::Random(stateLPFRand->get_ndy(), 2 * stateLPFRand->get_ndy()));
  ...
  Eigen::MatrixXd JrefLPF(
      Eigen::MatrixXd::Random(stateLPFNone->get_ndy(), 2 * stateLPFNone->get_ndy()));
