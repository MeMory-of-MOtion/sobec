...
rmodelComplete, geomModelComplete, visualModelComplete = pin.buildModelsFromUrdf(modelPath + URDF_SUBPATH, modelPath, pin.JointModelFreeFlyer())

# Take rotor inertia and gear ratio into account
pin.loadRotorParameters(rmodelComplete, modelPath + SRDF_SUBPATH, False)
rmodelComplete.armature = np.multiply(rmodelComplete.rotorInertia.flat, np.square(rmodelComplete.rotorGearRatio.flat))
...
pinocchioControlledJoints = [ i for (i,n) in enumerate(rmodelComplete.names) if n not in nonControlledJoints]
...
JointsToLockId = [ i for i in range(1,rmodelComplete.njoints) if i not in pinocchioControlledJoints]
...
supportContactModelLeft = crocoddyl.ContactModel6D(state, framePlacementLeft, actuation.nu, np.matrix([0, 50]).T)
contactModelDouble.addContact(leftFoot + "_contact", supportContactModelLeft)
framePlacementRight = crocoddyl.FramePlacement(rightFootId, rdata.oMf[rightFootId]) #rdata.oMi[1:][rightFootId]
supportContactModelRight = crocoddyl.ContactModel6D(state, framePlacementRight, actuation.nu, np.matrix([0, 50]).T)
...
xlb = np.vstack([-maxfloat * np.matrix(np.ones((6,1))), # dimension of the SE(3) manifold
                 rmodel.lowerPositionLimit[7:],
                 -maxfloat * np.matrix(np.ones((state.nv,1)))])
xub = np.vstack([maxfloat * np.matrix(np.ones((6,1))), # dimension of the SE(3) manifold
                 rmodel.upperPositionLimit[7:],
                 maxfloat * np.matrix(np.ones((state.nv,1)))])
bounds = crocoddyl.ActivationBounds(xlb,xub,1.)

limitCost = crocoddyl.CostModelResidual(state,crocoddyl.ActivationModelQuadraticBarrier(bounds),
...
minNforce = 200
maxNforce = 1200
wrenchConeFrameLeft = crocoddyl.WrenchCone(coneRotationLeft, mu, cone_box,4,True,minNforce,maxNforce)
wrenchConeFrameRight = crocoddyl.WrenchCone(coneRotationRight, mu, cone_box,4,True,minNforce,maxNforce)
...
wrenchConeResidualLeft = crocoddyl.ResidualModelContactWrenchCone(state,leftFootId,wrenchConeFrameLeft,actuation.nu)
wrenchConeResidualRight = crocoddyl.ResidualModelContactWrenchCone(state,rightFootId,wrenchConeFrameRight,actuation.nu)
wrenchConeCostLeft = crocoddyl.CostModelResidual(state,sobec.ActivationModelQuadRef(wrenchRef1),wrenchConeResidualLeft)
wrenchConeCostRight = crocoddyl.CostModelResidual(state,sobec.ActivationModelQuadRef(wrenchRef1),wrenchConeResidualRight)

wrenchConeResidualLeft2 = crocoddyl.ResidualModelContactWrenchCone(state,leftFootId,wrenchConeFrameLeft,actuation.nu)
wrenchConeResidualRight2 = crocoddyl.ResidualModelContactWrenchCone(state,rightFootId,wrenchConeFrameRight,actuation.nu)
wrenchConeCostLeft2 = crocoddyl.CostModelResidual(state,sobec.ActivationModelQuadRef(wrenchRefTwoSupports),wrenchConeResidualLeft2)
wrenchConeCostRight2 = crocoddyl.CostModelResidual(state,sobec.ActivationModelQuadRef(wrenchRefTwoSupports),wrenchConeResidualRight2)

#####  Cost for state and control

runningCosts = np.array([1000.,0.1,0.001,0,1e3,0.005,100])  #[1.,0.02,0.0004,0.0,1e3]
...
stateWeights = np.array(weightBasePos + weightLegPos * 2 + weightTorsoPos + weightArmLeftPos + weightArmRightPos +
                        weightBaseVel + weightLegVel * 2 + weightTorsoVel + weightArmLeftVel + weightArmRightVel)
...
xRegCost = crocoddyl.CostModelResidual(state,crocoddyl.ActivationModelWeightedQuad(np.matrix(stateWeights).T),xRegResidual)

uResidual = crocoddyl.ResidualModelControl(state,actuation.nu)
uRegCost = crocoddyl.CostModelResidual(state,crocoddyl.ActivationModelWeightedQuad(np.matrix(controlWeight).T),uResidual) 

#### Foot placement cost
startingPosLeftFoot = rdata.oMf[endEffectorIdLeft].copy()
startingPosRightFoot = rdata.oMf[endEffectorIdRight].copy()

residualPlacementRight = crocoddyl.ResidualModelFramePlacement(state,endEffectorIdRight,startingPosRightFoot,actuation.nu)
residualPlacementLeft = crocoddyl.ResidualModelFramePlacement(state,endEffectorIdLeft,startingPosLeftFoot,actuation.nu)

goalTrackingCostRight = crocoddyl.CostModelResidual(state,crocoddyl.ActivationModelQuadFlatLog(6,0.002),residualPlacementRight)
goalTrackingCostLeft = crocoddyl.CostModelResidual(state,crocoddyl.ActivationModelQuadFlatLog(6,0.002),residualPlacementLeft)

#### CoM velocity regularization
residualCoMVelocity = sobec.ResidualModelCoMVelocity(state, a2m(np.array([0,0,0])), actuation.nu)
...
dmodelRunning = crocoddyl.DifferentialActionModelContactFwdDynamics(state, actuation, contactModelDouble,runningCostModel,0,True)
...
runningCostModelLeftSwing.addCost("gripperPoseLeft", goalTrackingCostLeft, wFootPlacement )
...
dmodelRunningLeftSwing = crocoddyl.DifferentialActionModelContactFwdDynamics(state, actuation, contactModelRight,runningCostModelLeftSwing,0,True)
runningModelLeftSwing = crocoddyl.IntegratedActionModelEuler(dmodelRunningLeftSwing, DT)

runningCostModelRightSwing = crocoddyl.CostModelSum(state, actuation.nu)
runningCostModelRightSwing.addCost("gripperPoseRight", goalTrackingCostRight, wFootPlacement)
...
dmodelRunningRightSwing = crocoddyl.DifferentialActionModelContactFwdDynamics(state, actuation, contactModelLeft,runningCostModelRightSwing,0,True)
runningModelRightSwing = crocoddyl.IntegratedActionModelEuler(dmodelRunningRightSwing, DT)
...
xs0 = [xStart] * (T+1)
us0 = [ddp.problem.runningModels[0].quasiStatic(ddp.problem.runningDatas[0], xStart)] * T
...
robotId = p.loadURDF(URDF_FILENAME,robotStartPosition, robotStartOrientation, useFixedBase = False)
...
bulletJointNames = [ p.getJointInfo(robotId,i)[1].decode() for i in range(p.getNumJoints(robotId)) ]
JointIndicesComplete = [bulletJointNames.index(rmodelComplete.names[i]) for i in range(2,rmodelComplete.njoints) ]

#Joints controlled with crocoddyl
bulletControlledJoints = [ i for i in JointIndicesComplete if p.getJointInfo(robotId,i)[1].decode() not in  nonControlledJoints ]
...
visualShapeTarget = p.createVisualShape(shapeType=p.GEOM_BOX,
                        halfExtents=[0.1, 0.075, 0.05],
                        rgbaColor=[0.0, 0.0, 1.0, 1.0],
                        specularColor=[0.4, .4, 0],
                        visualFramePosition=[0.0, 0.0, 0.0])

sphereIdRight = p.createMultiBody(baseMass=0.0,
                       baseInertialFramePosition=[0, 0, 0],
                       baseVisualShapeIndex=visualShapeTarget,
                       basePosition=[startingPosRightFoot.translation[0],
                                     startingPosRightFoot.translation[1],
                                     startingPosRightFoot.translation[2]],
                       useMaximalCoordinates=True)
                       
sphereIdLeft = p.createMultiBody(baseMass=0.0,
                       baseInertialFramePosition=[0, 0, 0],
                       baseVisualShapeIndex=visualShapeTarget,
                       basePosition=[startingPosLeftFoot.translation[0],
                                     startingPosLeftFoot.translation[1],
                                     startingPosLeftFoot.translation[2]],
                       useMaximalCoordinates=True)
                       
visualShapeTargetCom = p.createVisualShape(shapeType=p.GEOM_BOX,
...
for s in range (T_total):
	...
	for k in range(Nc):
		...
		p.setJointMotorControlArray(robotId, bulletControlledJoints, controlMode=p.TORQUE_CONTROL, forces=torques)
		p.stepSimulation()
	...
	if (Tswitch2double >= 0 and Tswitch2double <= T1contact):
		if swingRightPhaseDouble:
			...
			targetFrameNow.translation[0] = startingPosRightFoot.translation[0] + xForward * float(T1contact - Tswitch2double) / float(T1contact)
			targetFrameNow.translation[1] = startingPosRightFoot.translation[1] - yCorrection * float(T1contact - Tswitch2double) / float(T1contact)
			targetFrameNow.translation[2] = startingPosRightFoot.translation[2] + foot_height * np.sin(float(T1contact - Tswitch2double) / float(T1contact) * TFootDepth * 3.14 / 180)
			residualPlacementRight.reference = targetFrameNow
				
		else:
			# Update left foot desired placement
			targetFrameNow = startingPosLeftFoot.copy()
			targetFrameNow.translation[0] = startingPosLeftFoot.translation[0] + xForward * float(T1contact - Tswitch2double) / float(T1contact)
			targetFrameNow.translation[1] = startingPosLeftFoot.translation[1] + yCorrection * float(T1contact - Tswitch2double) / float(T1contact)
			targetFrameNow.translation[2] = startingPosLeftFoot.translation[2] + foot_height * np.sin(float(T1contact - Tswitch2double) / float(T1contact) * TFootDepth * 3.14 / 180)
			residualPlacementLeft.reference = targetFrameNow
	
	p.resetBasePositionAndOrientation(sphereIdRight, posObj=[residualPlacementRight.reference.translation[0],
	                                 residualPlacementRight.reference.translation[1],
	                                 residualPlacementRight.reference.translation[2]],ornObj=np.array([0.0, 0.0, 0.0, 1.0]))
	p.resetBasePositionAndOrientation(sphereIdLeft, posObj=[residualPlacementLeft.reference.translation[0],
									 residualPlacementLeft.reference.translation[1],
									 residualPlacementLeft.reference.translation[2]],ornObj=np.array([0.0, 0.0, 0.0, 1.0]))
