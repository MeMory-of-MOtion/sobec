...
void test_partial_derivatives_against_numdiff(const boost::shared_ptr<sobec::IntegratedActionModelLPF>& model) {
  // create the corresponding data object and set the cost to nan
  const boost::shared_ptr<crocoddyl::ActionDataAbstract>& data = model->createData();
...
void test_partial_derivatives_action_model(ActionModelLPFTypes::Type iam_type,
...
void test_calc_alpha0_equivalent_euler(ActionModelLPFTypes::Type iam_type, DifferentialActionModelTypes::Type dam_type,
                                       PinocchioReferenceTypes::Type ref_type = PinocchioReferenceTypes::LOCAL,
                                       ContactModelMaskTypes::Type mask_type = ContactModelMaskTypes::Z) {
  // Create IAM LPF
  ActionModelLPFFactory factory_iam;
  ...
  boost::shared_ptr<crocoddyl::IntegratedActionModelEuler> modelEuler =
      boost::make_shared<crocoddyl::IntegratedActionModelEuler>(modelLPF->get_differential(), modelLPF->get_dt(),
                                                                true);
...
void test_calc_NONE_equivalent_euler(ActionModelLPFTypes::Type iam_type, DifferentialActionModelTypes::Type dam_type,
                                     PinocchioReferenceTypes::Type ref_type = PinocchioReferenceTypes::LOCAL,
                                     ContactModelMaskTypes::Type mask_type = ContactModelMaskTypes::Z) {
  // Create IAM LPF
  ActionModelLPFFactory factory_iam;
...
void test_calcDiff_explicit_equivalent_euler(ActionModelLPFTypes::Type iam_type,
                                             DifferentialActionModelTypes::Type dam_type,
                                             PinocchioReferenceTypes::Type ref_type = PinocchioReferenceTypes::LOCAL,
                                             ContactModelMaskTypes::Type mask_type = ContactModelMaskTypes::Z) {
  // Create IAM LPF
  ...
  for (std::size_t i = 0; i < non_lpf_torque_ids.size(); i++) {
    ...
    BOOST_CHECK(
        (Lxu_nonLPF.topRows(ndx).col(non_lpf_torque_ids[i]) - dataEuler->Lxu.col(non_lpf_torque_ids[i])).isZero(tol));
    for (std::size_t j = 0; j < non_lpf_torque_ids.size(); j++) {
      BOOST_CHECK((Luu_nonLPF(non_lpf_torque_ids[i], non_lpf_torque_ids[j]) -
                   dataEuler->Luu(non_lpf_torque_ids[i], non_lpf_torque_ids[j])) <= tol);
    }
    for (std::size_t j = 0; j < lpf_torque_ids.size(); j++) {
      BOOST_CHECK((Lxu_nonLPF.bottomRows(ntau)(j, non_lpf_torque_ids[i]) -
                   dataEuler->Luu(lpf_torque_ids[j], non_lpf_torque_ids[i])) <= tol);
    }
  }
  ...
  for (std::size_t i = 0; i < lpf_torque_ids.size(); i++) {
    BOOST_CHECK((Fu_LPF_term.col(i) - dataEulerTerminal->Fu.col(lpf_torque_ids[i])).isZero(tol));
  ...
  for (std::size_t i = 0; i < non_lpf_torque_ids.size(); i++) {
    BOOST_CHECK(
        (Fu_nonLPF_term.col(non_lpf_torque_ids[i]) - dataEulerTerminal->Fu.col(non_lpf_torque_ids[i])).isZero(tol));
    BOOST_CHECK((Lu_nonLPF_term(non_lpf_torque_ids[i]) - dataEulerTerminal->Lu(non_lpf_torque_ids[i])) <= tol);
    BOOST_CHECK(
        (Lxu_nonLPF_term.topRows(ndx).col(non_lpf_torque_ids[i]) - dataEulerTerminal->Lxu.col(non_lpf_torque_ids[i]))
            .isZero(tol));
    for (std::size_t j = 0; j < non_lpf_torque_ids.size(); j++) {
      BOOST_CHECK((Luu_nonLPF_term(non_lpf_torque_ids[i], non_lpf_torque_ids[j]) -
                   dataEulerTerminal->Luu(non_lpf_torque_ids[i], non_lpf_torque_ids[j])) <= tol);
    }
    for (std::size_t j = 0; j < lpf_torque_ids.size(); j++) {
      BOOST_CHECK((Lxu_nonLPF_term.bottomRows(ntau)(j, non_lpf_torque_ids[i]) -
                   dataEulerTerminal->Luu(lpf_torque_ids[j], non_lpf_torque_ids[i])) <= tol);
    }
  }
}
...
void register_action_model_unit_tests(ActionModelLPFTypes::Type iam_type, DifferentialActionModelTypes::Type dam_type,
  ...
  ts->add(
      BOOST_TEST_CASE(boost::bind(&test_partial_derivatives_action_model, iam_type, dam_type, ref_type, mask_type)));
  ...
  ts->add(
      BOOST_TEST_CASE(boost::bind(&test_calcDiff_explicit_equivalent_euler, iam_type, dam_type, ref_type, mask_type)));
