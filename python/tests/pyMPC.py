...
# -*- coding: utf-8 -*-
"""
Created on Tue May 24 15:48:02 2022
...
class CrocoWBC:
    def __init__(self, conf, design):
        ...
        self.full_cycle = self.generate_full_cycle_horizon()
        
        for time in range(self.full_cycle.ddp.problem.T):
    ...
    def generate_full_cycle_horizon(self):
        ...
        takeoff_RF = 0
        land_RF = takeoff_RF + self.conf.T1contact
        takeoff_LF = takeoff_RF + self.conf.Tstep
        land_LF = takeoff_LF + self.conf.T1contact
        
        for i in range(2*self.conf.Tstep):
            if i < land_RF:
                cycle.append({"support" : "left"})

            elif i < takeoff_LF:
                cycle.append({"support" : "double"})
                
            elif i < land_LF:
                cycle.append({"support" : "right"})
                
            else:
                cycle.append({"support" : "double"})
            
        return ReceidingHorizon(self.conf, self.design, self.x0, 
                                modeller, len(cycle), *cycle)
    ...
    def shapeState(self, q_current, v_current):
        ...
        return np.hstack((q_current[:7], qc,
                          v_current[:6], vc))
    ...
    def update_step_cycle_timing(self):
        ...
        if self.t_land_LF[0] < 0: self.t_land_LF += 2 * self.conf.Tstep
        if self.t_land_RF[0] < 0: self.t_land_RF += 2 * self.conf.Tstep    
        if self.t_takeoff_LF[0] < 0: self.t_takeoff_LF += 2 * self.conf.Tstep
        if self.t_takeoff_RF[0] < 0: self.t_takeoff_RF += 2 * self.conf.Tstep  
    ...
    def decide_actions(self, x0, is_feasible):
        ...
        self.horizon.ddp.solve(warm_xs, warm_us, 
                               self.conf.ddpIteration, is_feasible)
...
#            self.update_feet_ref_frame() 
            self.set_desired_feet_placement(time, self.conf.T - 1)
            
            # ~~ decide ~~ #
            self.decide_actions(x0, OL_MPC)
            
        # ~~ funtion to obtain x_ref by integrating xs[0] with us[0] during one tracking period.
        torques =  (self.horizon.ddp.us[0] + 
                    self.horizon.ddp.K[0] @ 
                    self.horizon.state.diff(x0, self.horizon.ddp.xs[0]))
        
        return torques
    
    def update_models(self):
        self.horizon.recede(self.full_cycle.IAM(0), self.full_cycle.data(0))
    ...
    def set_contacts(self, preview_time):
        ...
        if contacts[self.conf.leftFoot] and contacts[self.conf.rightFoot]:
            ...
            self.horizon.set_force_reference_LF(preview_time, 
                                                np.array([0,0, self.fz_ref/2, 0, 0,0]))
            self.horizon.set_force_reference_RF(preview_time,
                                                np.array([0,0,self.fz_ref/2, 0, 0,0]))
            ...
            self.horizon.set_force_reference_LF(preview_time,
                                                np.array([0,0,self.fz_ref, 0, 0,0]))
    ...
    def update_feet_ref_frame(self):
        if self.t_takeoff_LF[0] == 0:          
            ...
            frame_LF = crocoddyl.WrenchCone(orientationLF, self.conf.mu, self.conf.cone_box,4,True,self.conf.minNforce,self.conf.maxNforce)
            ...
            self.horizon.set_residual_references("left_wrench_cone", 
                                                 frame_LF, 
                                                 range(0, support_end))
            
        if self.t_takeoff_RF[0] == 0:
            orientationRF = self.design.get_RF_frame().copy().rotation.T
            frame_RF = crocoddyl.WrenchCone(orientationRF, self.conf.mu, self.conf.cone_box,4,True,self.conf.minNforce, self.conf.maxNforce)
            ...
            self.horizon.set_residual_references("right_wrench_cone",
                                                 frame_RF,
                                                 range(0, support_end))
    ...
    def set_desired_feet_placement(self, current_time, preview_time):  
        ...
        global_time = current_time / self.conf.Nc + preview_time
        LF.translation = self.foot_trajectory(global_time, next_land_LF-preview_time, LF)
        RF.translation = self.foot_trajectory(global_time, next_land_RF-preview_time, RF)
#        print(self.RF_sample.translation)
        self.horizon.set_residual_reference(preview_time,
                                            "left_foot_place", LF.copy())
        self.horizon.set_residual_reference(preview_time,
                                            "right_foot_place", RF.copy())
        
    def foot_trajectory(self, time, time_to_land, pose, trajectory = "sine"):
        tmax = self.conf.T1contact
        t = time_to_land - 4 # minus landing_advance
        
        if trajectory == "sine":
            z = 0 if t<0 or t>tmax-8 else (np.sin(t*np.pi/(tmax-8))) * 0.04 ### SINE
        else:    
            z = 0 if t<0 or t>tmax-8 else (1-np.cos(2*t*np.pi/(tmax-8))) * 0.02 ### SMOTHER
    ...
    def estimate_deflection(self, command, hip_yawls=[0,0]):
        ...
        delta = compute_deflection(
            tau, self.delta, self.conf.H_stiff, 
            self.conf.H_damp, self.conf.simu_period
        )
...
#            delta_dot = self.movAverage(delta_dot, 0.01, "flex_rate")
    ...
    def take_deflection(self, device):pass
    ...
    def joint_estimation(self, real_state, command=None, device=None):
        ...
        if self.conf.exact_deflection:
            ...
            self.delta, self.delta_dot = self.estimate_deflection(command, 
                                                                  hip_yawl)
