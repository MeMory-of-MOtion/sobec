...
class CrocoWBC:
    ...
    def generate_full_cycle_horizon(self):
        ...
        return ReceidingHorizon(
            self.conf, self.design, self.x0, modeller, len(cycle), *cycle
        )
    ...
    def set_desired_feet_placement(self, current_time, preview_time):
        ...
        LF.translation = self.foot_trajectory(
            global_time, next_land_LF - preview_time, LF
        )
        RF.translation = self.foot_trajectory(
            global_time, next_land_RF - preview_time, RF
        )
