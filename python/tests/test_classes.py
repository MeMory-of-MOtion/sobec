...
class ClasesTestCase(unittest.TestCase):
    def setUp(self):
        ...
        x0 = py_mpc.shapeState(py_design.rmodelComplete.q0, 
        ...
        MM_conf = dict(timeStep = config.DT,
        ...
        H_conf = dict(leftFootName = config.lf_frame_name, 
                      rightFootName = config.rf_frame_name)
        ...
        wbc_conf = dict(horizonSteps = config.preview_steps,
    ...
    def test_RobotWrapper(self):
        ...
        self.assertTrue((self.py_design.get_LF_frame().homogeneous ==
                               self.design.get_LF_frame().homogeneous).all())
        self.assertTrue((self.py_design.get_RF_frame().homogeneous ==
                               self.design.get_RF_frame().homogeneous).all())
        self.assertEqual(self.py_design.pinocchioControlledJoints, self.design.get_controlledJointsIDs().tolist())
        ...
        self.assertTrue((self.py_design.q0Complete == self.design.get_q0Complete()).all())
        ...
        self.assertTrue((self.py_design.rmodel.defaultState == self.design.get_x0()).all())
        ...
        self.assertEqual(self.py_design.rmodelComplete.nv, self.design.get_rModelComplete().nv)
        self.assertEqual(self.py_design.rmodelComplete.nq, self.design.get_rModelComplete().nq)
        
        self.py_design.update_reduced(self.py_design.rmodel.q0)
        self.design.updateReducedModel(self.py_design.rmodel.q0*0)
        
        self.assertTrue(not(self.py_design.get_LF_frame().homogeneous ==
                               self.design.get_LF_frame().homogeneous).all())
        self.assertTrue(not(self.py_design.get_RF_frame().homogeneous ==
                               self.design.get_RF_frame().homogeneous).all())
...
#        self.assertTrue(self.horizon.ddp.solve())
    ...
    def test_MPC(self):
        ...
        q = np.random.rand(nq)
        v = np.random.rand(nv)
        self.assertTrue((self.mpc.shapeState(q, v) == 
                         self.py_mpc.shapeState(q, v)).all())
