...
def define_support_contact(conf, design, state, actuation, support, contacts):
    ...
    if support == "left" or support == "double":
        frameLeftFoot = crocoddyl.FramePlacement(design.leftFootId, design.get_LF_frame().copy())
        left6Dcontact = crocoddyl.ContactModel6D(state, frameLeftFoot, actuation.nu, np.array([0, 50]))
        contacts.addContact(conf.leftFoot, left6Dcontact)
        
    if support == "right" or support == "double":
        frameRightFoot = crocoddyl.FramePlacement(design.rightFootId, design.get_RF_frame().copy())
        right6Dcontact = crocoddyl.ContactModel6D(state, frameRightFoot, actuation.nu, np.array([0, 50]))
...
def wrench_cone(conf, design, state, actuation, support, costs):
    ...
    if support == "left" or support == "double":  
        ...
        wrenchConeFrameLeft = crocoddyl.WrenchCone(LF_orientation, conf.mu, conf.cone_box,4,True,conf.minNforce,conf.maxNforce)
    ...
    if support == "right" or support == "double":
        ...
        wrenchConeFrameRight = crocoddyl.WrenchCone(RF_orientation, conf.mu, conf.cone_box,4,True,conf.minNforce,conf.maxNforce)
...
def joint_limits(conf, design, state, actuation, costs):
    ...
    xlb = np.hstack([-np.Inf * np.ones(6), # dimension of the SE(3) manifold
                     design.rmodel.lowerPositionLimit[7:],
                     -np.Inf * np.ones(state.nv)])
    xub = np.hstack([np.Inf * np.ones(6), # dimension of the SE(3) manifold
                     design.rmodel.upperPositionLimit[7:],
                     np.Inf * np.ones(state.nv)])
    bounds = crocoddyl.ActivationBounds(xlb,xub,1.)
    
    jointLimitResiduals = crocoddyl.ResidualModelState(state,
                                                       np.zeros(state.nx), 
                                                       actuation.nu)
    limitCost = crocoddyl.CostModelResidual(state,
...
def control_n_state_regularization(conf, design, state, actuation, costs):
    ...
    xRegResidual = crocoddyl.ResidualModelState(state, design.rmodel.defaultState, actuation.nu)
    xRegCost = crocoddyl.CostModelResidual(state, crocoddyl.ActivationModelWeightedQuad(conf.stateWeights),
                                           xRegResidual)
    uResidual = crocoddyl.ResidualModelControl(state, actuation.nu)
    uRegCost = crocoddyl.CostModelResidual(state, crocoddyl.ActivationModelWeightedQuad(conf.controlWeight),
                                           uResidual) 
...
#    goalTranslationLeft[2] -= deepFootHeight
    ...
    boundsXYleft = crocoddyl.ActivationBounds(lowYleft, upperYleft, 1.)
    boundsXYright = crocoddyl.ActivationBounds(lowYright, upperYright, 1.)
    ...
    translationZCostRight = crocoddyl.CostModelResidual(
            state, crocoddyl.ActivationModelWeightedQuad(transZWeights), residualTranslationZRight
            )
    translationZCostLeft = crocoddyl.CostModelResidual(
            state,crocoddyl.ActivationModelWeightedQuad(transZWeights),residualTranslationZLeft
            )
    
    translationXYCostRight = crocoddyl.CostModelResidual(state,crocoddyl.ActivationModelQuadraticBarrier(boundsXYright),residualTranslationXYRight)
    translationXYCostLeft = crocoddyl.CostModelResidual(state,crocoddyl.ActivationModelQuadraticBarrier(boundsXYleft),residualTranslationXYLeft)

    costs.addCost("left_foot_XY_Translation", translationXYCostLeft, conf.wFootXYTrans )
    costs.addCost("right_foot_XY_Translation", translationXYCostRight, conf.wFootXYTrans )
...
def feet_orientation(conf, design, state, actuation, costs):
    residualRotationRight = crocoddyl.ResidualModelFrameRotation(state, design.rightFootId, np.eye(3), actuation.nu)
    residualRotationLeft = crocoddyl.ResidualModelFrameRotation(state, design.leftFootId, np.eye(3), actuation.nu)
...
def ground_contact(conf, design, state, actuation, costs):
    ...
    residualPairCollisionLeft = crocoddyl.ResidualModelVelCollision(
            state,actuation.nu, design.geomModel, 1, 
            design.leftFootId, design.WORLD, 0.01
            )
    leftGroundPenetrationCost = crocoddyl.CostModelResidual(state, residualPairCollisionLeft)
    
    residualPairCollisionRight = crocoddyl.ResidualModelVelCollision(
            state,actuation.nu, design.geomModel, 0, 
            design.rightFootId, design.WORLD, 0.01
            )
    rightGroundPenetrationCost = crocoddyl.CostModelResidual(state, residualPairCollisionRight)
