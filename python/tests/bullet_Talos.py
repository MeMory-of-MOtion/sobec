...
class BulletTalos():
    def __init__(self, conf, rmodelComplete):
        ...
        self.robotId = p.loadURDF(conf.URDF_FILENAME,robotStartPosition, 
                             robotStartOrientation, useFixedBase = False)
        ...
        bulletJointNames = [ p.getJointInfo(self.robotId,i)[1].decode() for i in range(p.getNumJoints(self.robotId)) ]
        self.JointIndicesComplete = [bulletJointNames.index(rmodelComplete.names[i]) for i in range(2,rmodelComplete.njoints) ]

        #Joints controlled with crocoddyl
        self.bulletControlledJoints = [ i for i in self.JointIndicesComplete if p.getJointInfo(self.robotId,i)[1].decode() not in  conf.blocked_joints ]
    ...
    def initializeJoints(self, q0CompleteStart):
        ...
        for i in range(len(initial_joint_positions)):
            ...
            p.resetJointState(self.robotId, self.JointIndicesComplete[i], 
                              initial_joint_positions[i])
            
    def execute(self, torques):
        p.setJointMotorControlArray(self.robotId, self.bulletControlledJoints, 
                                    controlMode=p.TORQUE_CONTROL, forces=torques)
        p.stepSimulation()
        
    def measureState(self):
        jointStates = p.getJointStates(self.robotId, self.JointIndicesComplete) # State of all joints
        baseState   = p.getBasePositionAndOrientation(self.robotId)
        baseVel = p.getBaseVelocity(self.robotId)
        
        # Joint vector for Pinocchio
        q = np.hstack([baseState[0], baseState[1], [jointStates[i_joint][0] 
                                    for i_joint in range(len(jointStates))]])
        v = np.hstack([baseVel[0], baseVel[1], [jointStates[i_joint][1] 
                                    for i_joint in range(len(jointStates))]])
    ...
    def showTargetToTrack(self, LF_pose, RF_pose):
        ...
        self.sphereIdRight = p.createMultiBody(baseMass=0.0,
                               baseInertialFramePosition=[0, 0, 0],
                               baseVisualShapeIndex=visualShapeTarget,
                               basePosition=[RF_pose.translation[0],
                                             RF_pose.translation[1],
                                             RF_pose.translation[2]],
                               useMaximalCoordinates=True)
        
        self.sphereIdLeft = p.createMultiBody(baseMass=0.0,
                               baseInertialFramePosition=[0, 0, 0],
                               baseVisualShapeIndex=visualShapeTarget,
                               basePosition=[LF_pose.translation[0],
                                             LF_pose.translation[1],
                                             LF_pose.translation[2]],
                               useMaximalCoordinates=True)
                               
        self.visualShapeTargetCom = p.createVisualShape(shapeType=p.GEOM_BOX,
    ...
    def moveMarkers(self, LF_pose, RF_pose):
        ...
        p.resetBasePositionAndOrientation(self.sphereIdRight, 
                                           posObj=[RF_pose.translation[0], 
                                                   RF_pose.translation[1],
                                                   0],
                                                   ornObj=np.array([0.0, 0.0, 0.0, 1.0]))
        p.resetBasePositionAndOrientation(self.sphereIdLeft, 
                                           posObj=[LF_pose.translation[0],
                                                   LF_pose.translation[1],
                                                   0],
                                                   ornObj=np.array([0.0, 0.0, 0.0, 1.0]))
