...
class PinTalos:
    def __init__(self, conf):
        ...
        rmodelComplete = pin.buildModelsFromUrdf(conf.modelPath + conf.URDF_SUBPATH,
                                                 conf.modelPath, pin.JointModelFreeFlyer())[0]

        # Take rotor inertia and gear ratio into account
        pin.loadRotorParameters(rmodelComplete, 
                                conf.modelPath + conf.SRDF_SUBPATH, False)
        rmodelComplete.armature = np.multiply(rmodelComplete.rotorInertia.flat,
                                              np.square(rmodelComplete.rotorGearRatio.flat))
        pin.loadReferenceConfigurations(rmodelComplete,conf.modelPath + conf.SRDF_SUBPATH, False)
        ...
        pinocchioControlledJoints = [ i for (i,n) in enumerate(rmodelComplete.names) 
        ...
        JointsToLockId = [ i for i in range(1,rmodelComplete.njoints) 
...
#        pin.loadReferenceConfigurations(rmodel, conf.modelPath + conf.SRDF_SUBPATH)
        ...
        rmodel.defaultState = np.hstack([q0, np.zeros(rmodel.nv)]) ## TODO: remove this default state and use a varing state.
