...
class ReceidingHorizon:
    def __init__(self, conf, design, x0, modeller, N, *forms):
        ...
        ia_models = [modeller(conf, design, state, actuation, **(forms[i])) 
                     for i in range(N)]
    ...
    def get_contacts(self, time):
        ...
        return {self.conf.leftFoot: self.conf.leftFoot in active,
    ...
    def recede(self, new_model=None, new_data=None):
        ...
        aux_data = new_data if new_data else (new_model.createData() 
                                              if new_model else self.data(0)) 
