...
namespace sobec {
    namespace python {
        ...
        void initialize(OCP &self, 
                        const bp::dict &settings, 
                        const bp::dict &model_settings,
                        const bp::dict &design,
        ...
        HorizonManager get_horizon(OCP &self){
			return self.get_horizon();
		}

        void exposeOCP() {

            bp::class_<OCP>("OCP", bp::init<>())
                .def("initialize", &initialize, bp::args("self", "settings", "model_settings", "design", "q0", "v0"), "The posture required here is the full robot posture in the order of pin0cchio")
