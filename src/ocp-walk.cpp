...
OCPWalk::buildRunningModel(
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (int k = 0; k < robot->contactIds.size();
      ...
      boost::shared_ptr<ContactModel6D> contact =
    ...
    boost::shared_ptr<ResidualModelState> xRegResidual =
        boost::make_shared<ResidualModelState>(state, robot->x0,
                                               actuation->get_nu());
    boost::shared_ptr<ActivationModelWeightedQuad> activation_state =
        boost::make_shared<ActivationModelWeightedQuad>(
            params->stateImportance * params->stateImportance);
    boost::shared_ptr<CostModelResidual> xRegCost =
        boost::make_shared<CostModelResidual>(state, activation_state,
                                              xRegResidual);
    ...
    boost::shared_ptr<CostModelResidual> uRegCost =
        boost::make_shared<CostModelResidual>(state, activation_ctrl,
                                              uResidual);
    ...
    boost::shared_ptr<ResidualModelCoMPosition> comResidual =
        boost::make_shared<ResidualModelCoMPosition>(state, robot->com0,
                                                     actuation->get_nu());
    ...
    boost::shared_ptr<ResidualModelCoMVelocity> comVelResidual =
        boost::make_shared<ResidualModelCoMVelocity>(state, params->vcomRef,
                                                     actuation->get_nu());
    ...
    for (int k = 0; k < robot->contactIds.size();
         ++k)  // k, cid in enumerate(robot.contactIds):
    {
      if (contact_patterns(k, i) == 0.) continue;
      ...
      w_cop << value, value;
      boost::shared_ptr<CostModelResidual> copResidual =
          boost::make_shared<ResidualModelCenterOfPressure>(
              state, cid, actuation->get_nu());
      boost::shared_ptr<CostModelResidual> copAct =
          boost::make_shared<ActivationModelWeightedQuad>(w_cop);
      boost::shared_ptr<CostModelResidual> copCost =
      ...
      corners << value, value;
      boost::shared_ptr<CostModelResidual> cone =
          boost::make_shared<WrenchCone>(Eigen::Matrix3D::Identity(), 1000,
                                         corners, 4, true, 1, 10000);
      boost::shared_ptr<CostModelResidual> coneCost =
          boost::make_shared<ResidualModelContactWrenchCone>(
      ...
      boost::shared_ptr<CostModelResidual> coneAct =
      ...
      costs->addCost(robot->model->frames[cid].name + "_cone", coneCost,
      ...
      // formulation.
      boost::shared_ptr<CostModelResidual> coneAxisResidual =
      ...
      w[2] = 0.0;
      boost::shared_ptr<CostModelResidual> coneAxisAct =
          boost::make_shared<ActivationModelWeightedQuad>(w);
      boost::shared_ptr<CostModelResidual> coneAxisCost =
          boost::make_shared<CostModelResidual>(state, coneAxisAct,
                                                coneAxisResidual);
      ...
      boost::shared_ptr<CostModelResidual> forceRefResidual =
          boost::make_shared<ResidualModelContactForce>(
              state, cid, pinocchio::Force(referenceForces[t, k]), 6,
              actuation->get_nu());
      boost::shared_ptr<CostModelResidual> forceRefCost =
