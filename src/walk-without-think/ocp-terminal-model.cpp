...
AMA OCPWalk::buildTerminalModel() {
  ...
  for (int k = 0; k < robot->contactIds.size(); ++k) {  // k, cid in enumerate(robot.contactIds):
    ...
    auto contact = boost::make_shared<ContactModel6D>(state, cid, pinocchio::SE3::Identity(), actuation->get_nu(),
  ...
  auto stateTerminalResidual = boost::make_shared<ResidualModelState>(state, stateTerminalTarget, actuation->get_nu());
  ...
  auto stateTerminalCost = boost::make_shared<CostModelResidual>(state, activation_state, stateTerminalResidual);
