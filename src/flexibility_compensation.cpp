...
Flex::Flex(const FlexSettings &settings) { settings_ = settings; }

void Flex::initialize(const FlexSettings &settings) { settings_ = settings; }

eVector2 Flex::computeDeflection(const eArray2 &torques, const eArray2 &delta0,
...
eVector3 Flex::equivalentAngles(const eMatrixRot &fullRotation) {
  ...
  return eVector3(qz_,
                  atan2(fullRotation(2, 1), -fullRotation(0, 1) * sin(qz_) +
