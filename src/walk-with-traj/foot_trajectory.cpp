...
ndcurves::polynomial1_t FootTrajectory::build_height_predef_trajectory(
    const double p, const double t_init, const double t_end,
  ...
  const double jerk_coef =
      compute_required_jerk(offset, duration);  // it actually jerk/6
...
double compute_predef_height(const double duration_predef,
                             const double t_middle, const double p_max) {
  double p =
      p_max / (1. + 4. * t_middle / duration_predef +
...
ndcurves::bezier1_t FootTrajectory::build_height_predef_lift(
    const double p_init, const double t_init, const double t_end) {
  /*
  ...
  const double p_end =
      p_init + compute_predef_height(T, t_end_ - t_init_, lift_offset_);
...
ndcurves::bezier1_t FootTrajectory::build_height_predef_landing(
    const double p_end, const double t_init, const double t_end) {
  /*
  ...
  const double T(t_end - t_init);
  const double p_init =
      p_end + compute_predef_height(T, t_end_ - t_init_, land_offset_);
  const int n(4);  // degree of this curve
  ndcurves::bezier1_t::t_point_t coeffs(n + 1);
  coeffs[0] = ndcurves::point1_t(p_init);
...
ndcurves::bezier1_t FootTrajectory::build_height_middle_trajectory(
    const double& t_init, const double& t_end, const double p0, const double v0,
    const double a0, const double j0, const double p1, const double v1,
  ...
  coeffs[2] = ndcurves::point1_t(
      (n * n * p0 - n * p0 + 2. * n * v0 * T - 2. * v0 * T + a0 * T * T) /
      (n * (n - 1.)));
  coeffs[3] =
      ndcurves::point1_t((n * n * p0 - n * p0 + 3. * n * v0 * T - 3. * v0 * T +
                          3. * a0 * T * T + j0 * T * T * T / (n - 2)) /
                         (n * (n - 1.)));
  coeffs[4] =
      ndcurves::point1_t((n * n * p1 - n * p1 - 3 * n * v1 * T + 3 * v1 * T +
                          3 * a1 * T * T - j1 * T * T * T / (n - 2)) /
                         (n * (n - 1)));
  coeffs[5] = ndcurves::point1_t(
      (n * n * p1 - n * p1 - 2 * n * v1 * T + 2 * v1 * T + a1 * T * T) /
      (n * (n - 1)));
...
void FootTrajectory::build_height_trajectory_minjerk(
  ...
  const double mid_time =
      (t_init_ + t_end_) / 2.;  // time at the apex for the initial trajectory
  ...
  if (v_init == 0. && a_init == 0. && j_init == 0.) {
    ...
    {
      height_traj_.add_curve(ndcurves::polynomial1_t::MinimumJerk(
          ndcurves::point1_t(z_init), ndcurves::point1_t(z_apex), t_init,
          mid_time));
      height_traj_.add_curve(ndcurves::polynomial1_t::MinimumJerk(
          ndcurves::point1_t(z_apex), ndcurves::point1_t(z_end), mid_time,
          t_end));
    } else {
      height_traj_.add_curve(ndcurves::polynomial1_t::MinimumJerk(
          ndcurves::point1_t(z_init), ndcurves::point1_t(z_end), t_init,
          t_end));
    }
  } else {
                                  ...
                                  // 0 vel and acc. @TODO take that as param
    {
      height_traj_.add_curve(
          build_height_middle_trajectory(t_init, mid_time, z_init, v_init,
                                         a_init, j_init, z_apex, 0., 0., 0.));
      height_traj_.add_curve(ndcurves::polynomial1_t::MinimumJerk(
          ndcurves::point1_t(z_apex), ndcurves::point1_t(z_end), mid_time,
          t_end));
    } else {
      height_traj_.add_curve(build_height_middle_trajectory(
          t_init, t_end, z_init, v_init, a_init, j_init, z_end, 0., 0., 0.));
    }
  }
}

void FootTrajectory::build_height_trajectory(
    const double& t_init, const double& t_end, const double& z_init,
    const double& z_end, const double& v_init, const double& a_init,
    const double& j_init) {
  height_traj_ = ndcurves::piecewise1_t();
  ...
  if (t_init < lift_end_time_) {
    ...
    // update
    lift_traj =
        build_height_predef_trajectory(pose_init_.translation().z(), t_init_,
  ...
  // landing_advance_);
  land_traj = build_height_predef_trajectory(
      z_end, land_begin_time_, t_end - landing_advance_, land_offset_, false);
  ...
  height_traj_.add_curve(build_height_middle_trajectory(
      t_begin_middle, land_begin_time_, p0, v0, a0, j0, p1, v1, a1, j1));
  height_traj_.add_curve(land_traj);
  // add final constant :
  if (height_traj_.max() < t_end) {
    const ndcurves::point1_t height_land(land_traj(land_traj.max()));
    height_traj_.add_curve(ndcurves::polynomial1_t(height_land, height_land,
                                                   height_traj_.max(), t_end));
  }
  /*
...
void FootTrajectory::build_translation_trajectory(
  ...
  if (translation_traj.min() > t_init) {
    translation_traj_.add_curve(
        ndcurves::constant2_t(p_init, t_init, translation_traj.min()));
  }
  translation_traj_.add_curve(translation_traj);

  if (translation_traj.max() < t_end) {
    translation_traj_.add_curve(
        ndcurves::constant2_t(p_end, translation_traj.max(), t_end));
  }
}
void FootTrajectory::build_yaw_trajectory(
  ...
  if (t_init < lift_end_time_)
    ...
    t_begin_middle = t_init;
  ndcurves::polynomial1_t yaw_traj(
      build_yaw_middle_trajectory(t_begin_middle, land_begin_time_, yaw_init,
                                  yaw_end, delta_yaw_init, v_init, a_init));
  ...
  if (yaw_traj.min() > t_init) {
    ...
    yaw_traj_.add_curve(
        ndcurves::constant1_t(zero_yaw, t_init, yaw_traj.min()));
  }
...
ndcurves::polynomial2_t FootTrajectory::build_translation_middle_trajectory(
  ...
  if (v_init.isZero() && a_init.isZero())
    ...
    return ndcurves::polynomial2_t(p_init, v_init, a_init, p_end,
                                   eVector2::Zero(), eVector2::Zero(), t_init,
                                   t_end);
  }
}
...
ndcurves::polynomial1_t FootTrajectory::build_yaw_middle_trajectory(
  ...
  if (v_init == 0. && a_init == 0.)
    return ndcurves::polynomial1_t::MinimumJerk(
        ndcurves::point1_t(delta_yaw_init), ndcurves::point1_t(delta_yaw_end),
        t_init, t_end);
  else
...
void FootTrajectory::generate(const double& t_init, const double& t_end,
                              ...
                              const bool& constant) {
  assert(t_end > t_init &&
         "FootTrajectory::generateFeetTraj(): t_end > t_init");
  t_init_ = t_init;
  t_end_ = t_end;
  ...
  if (!constant) {
    ...
    build_height_trajectory_minjerk(t_init, t_end, pose_init.translation().z(),
                                    z_end);
    const eVector2 xy_init(pose_init.translation().x(),
                           pose_init.translation().y()),
...
void FootTrajectory::update(const double time,
                            const pinocchio::SE3& new_pose_end) {
  if (constant_) {
    throw std::runtime_error(
        "FootTrajectory::update : Cannot update a constant trajectory.");
  }
  if (time < min() || time > max()) {
    throw std::runtime_error(
        "FootTrajectory::update : new initial time is out of range");
  }
  // get previous goal and check if it's far from the new one :
  previous_pose_end_ = end();
  if (abs(new_pose_end.translation().x() -
          previous_pose_end_.translation().x()) >= 0.01 ||
      abs(new_pose_end.translation().y() -
          previous_pose_end_.translation().y()) >=
          0.01) {                   /*
    ...
    build_yaw_trajectory(time, t_end_, Spatial::extractYaw(pose_init_),
                         Spatial::extractYaw(pose_end_), init_yaw, init_yaw_d,
                         init_yaw_dd);
    build_translation_trajectory(time, t_end_, init_xy,
                                 pose_end_.translation().head<2>(), init_xy_d,
                                 init_xy_dd);

    build_height_trajectory_minjerk(time, t_end_, init_height,
                                    pose_end_.translation().z(), init_height_d,
...
pinocchio::SE3 FootTrajectory::compute(const double& time) {
  ...
  ret.rotation(pose_init_.rotation() *
               Spatial::matrixRollPitchYaw(0., 0., yaw));
  // set translation
...
pinocchio::SE3 FootTrajectory::average(const pinocchio::SE3& a,
