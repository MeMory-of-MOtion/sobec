...
void RobotDesigner::initialize(const RobotDesignerSettings &settings) {
  ...
  for (std::vector<std::string>::const_iterator it = settings_.controlledJointsNames.begin();
  ...
  for (std::vector<std::string>::const_iterator it = rModelComplete_.names.begin() + 1;
       it != rModelComplete_.names.end(); ++it) {
    const std::string &joint_name = *it;
    if (std::find(settings_.controlledJointsNames.begin(), settings_.controlledJointsNames.end(), joint_name) ==
...
void RobotDesigner::addToeAndHeel(const double &heel_translation, const double &toe_translation) {
  ...
  pinocchio::Frame toeFrameLeft(rModel_.frames[leftFootId_].name + "_" + "toe", rModel_.frames[leftFootId_].parent,
                                rModel_.frames[leftFootId_].previousFrame,
                                rModel_.frames[leftFootId_].placement * toePlacement, pinocchio::OP_FRAME);
  pinocchio::Frame toeFrameRight(rModel_.frames[rightFootId_].name + "_" + "toe", rModel_.frames[rightFootId_].parent,
                                 rModel_.frames[rightFootId_].previousFrame,
                                 rModel_.frames[rightFootId_].placement * toePlacement, pinocchio::OP_FRAME);
  ...
  pinocchio::Frame heelFrameLeft(rModel_.frames[leftFootId_].name + "_" + "heel", rModel_.frames[leftFootId_].parent,
                                 rModel_.frames[leftFootId_].previousFrame,
                                 rModel_.frames[leftFootId_].placement * heelPlacement, pinocchio::OP_FRAME);
  pinocchio::Frame heelFrameRight(rModel_.frames[rightFootId_].name + "_" + "heel",
                                  rModel_.frames[rightFootId_].parent, rModel_.frames[rightFootId_].previousFrame,
                                  rModel_.frames[rightFootId_].placement * heelPlacement, pinocchio::OP_FRAME);
...
const pinocchio::SE3 &RobotDesigner::get_LF_frame() { return rData_.oMf[leftFootId_]; }
...
void RobotDesigner::changeInertia(const size_t &inertia_id, const double &offset) {
