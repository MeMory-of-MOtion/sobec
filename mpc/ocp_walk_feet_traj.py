...
class OCP:
    def __init__(self,rmodel,verbose=True):
        ...
        supportContactModelLeft = croc.ContactModel6D(state, framePlacementLeft, actuation.nu, np.array([0, 50]))
        contactModelDouble.addContact(self.rmodel.frames[leftFootId].name + "_contact", supportContactModelLeft)
        framePlacementRight = croc.FramePlacement(rightFootId, rdata.oMf[rightFootId]) #rdata.oMi[1:][rightFootId]
        supportContactModelRight = croc.ContactModel6D(state, framePlacementRight, actuation.nu, np.array([0, 50]))
        contactModelDouble.addContact(self.rmodel.frames[rightFootId].name + "_contact", supportContactModelRight)
        
        contactModelLeft = croc.ContactModelMultiple(state,actuation.nu)
        contactModelLeft.addContact(self.rmodel.frames[leftFootId].name + "_contact", supportContactModelLeft)
        
        contactModelRight = croc.ContactModelMultiple(state,actuation.nu)
        contactModelRight.addContact(self.rmodel.frames[rightFootId].name + "_contact", supportContactModelRight)

        ######  Create the cost functions
        
        ######  Cost for self-collision
        xlb = np.concatenate([-float_info.max * np.ones(6), # dimension of the SE(3) manifold
                              self.rmodel.lowerPositionLimit[7:],
                              -float_info.max * np.ones(state.nv)])
        xub = np.concatenate([float_info.max * np.ones(6), # dimension of the SE(3) manifold
                              self.rmodel.upperPositionLimit[7:],
                              float_info.max * np.ones(state.nv)])
        bounds = croc.ActivationBounds(xlb,xub,1.)
        
        limitCost = croc.CostModelResidual(state,croc.ActivationModelQuadraticBarrier(bounds),
        ...
        wrenchConeFrameLeft = croc.WrenchCone(coneRotationLeft, mu, cone_box,4,True,minNforce,maxNforce)
        wrenchConeFrameRight = croc.WrenchCone(coneRotationRight, mu, cone_box,4,True,minNforce,maxNforce)
        boundsFrictionLeft = croc.ActivationBounds(wrenchConeFrameLeft.lb,wrenchConeFrameLeft.ub,1.)
        boundsFrictionRight = croc.ActivationBounds(wrenchConeFrameRight.lb,wrenchConeFrameRight.ub,1.)
        ...
        wrenchConeResidualLeft = croc.ResidualModelContactWrenchCone(state,leftFootId,wrenchConeFrameLeft,actuation.nu)
        wrenchConeResidualRight = croc.ResidualModelContactWrenchCone(state,rightFootId,wrenchConeFrameRight,actuation.nu)
        wrenchConeCostLeft = croc.CostModelResidual(state,sobec.ActivationModelQuadRef(wrenchRef1),wrenchConeResidualLeft)
        wrenchConeCostRight = croc.CostModelResidual(state,sobec.ActivationModelQuadRef(wrenchRef1),wrenchConeResidualRight)
        
        wrenchConeResidualLeft2 = croc.ResidualModelContactWrenchCone(state,leftFootId,wrenchConeFrameLeft,actuation.nu)
        wrenchConeResidualRight2 = croc.ResidualModelContactWrenchCone(state,rightFootId,wrenchConeFrameRight,actuation.nu)
        wrenchConeCostLeft2 = croc.CostModelResidual(state,sobec.ActivationModelQuadRef(wrenchRefTwoSupports),wrenchConeResidualLeft2)
        wrenchConeCostRight2 = croc.CostModelResidual(state,sobec.ActivationModelQuadRef(wrenchRefTwoSupports),wrenchConeResidualRight2)
        ...
        xRegCost = croc.CostModelResidual(state,croc.ActivationModelWeightedQuad(np.array(stateWeights)),xRegResidual)
        
        uResidual = croc.ResidualModelControl(state,actuation.nu)
        uRegCost = croc.CostModelResidual(state,croc.ActivationModelWeightedQuad(np.array(controlWeight)),uResidual) 
        
        #### Foot placement cost
        startingPosLeftFoot = rdata.oMf[leftFootId].copy()
        startingPosRightFoot = rdata.oMf[rightFootId].copy()
        
        residualPlacementRight = croc.ResidualModelFramePlacement(state,rightFootId,startingPosRightFoot,actuation.nu)
        residualPlacementLeft = croc.ResidualModelFramePlacement(state,leftFootId,startingPosLeftFoot,actuation.nu)
        
        goalTrackingCostRight = croc.CostModelResidual(state,croc.ActivationModelQuadFlatLog(6,0.002),residualPlacementRight)
        goalTrackingCostLeft = croc.CostModelResidual(state,croc.ActivationModelQuadFlatLog(6,0.002),residualPlacementLeft)
        
        #### CoM velocity regularization
        residualCoMVelocity = sobec.ResidualModelCoMVelocity(state, np.array([0,0,0]), actuation.nu)
        ...
        dmodelRunning = croc.DifferentialActionModelContactFwdDynamics(state, actuation, contactModelDouble,runningCostModel,0,True)
        ...
        dmodelRunningLeftSwing = croc.DifferentialActionModelContactFwdDynamics(state, actuation, contactModelRight,runningCostModelLeftSwing,0,True)
        runningModelLeftSwing = croc.IntegratedActionModelEuler(dmodelRunningLeftSwing, DT)
        ...
        dmodelRunningRightSwing = croc.DifferentialActionModelContactFwdDynamics(state, actuation, contactModelLeft,runningCostModelRightSwing,0,True)
        runningModelRightSwing = croc.IntegratedActionModelEuler(dmodelRunningRightSwing, DT)
        
        problem = croc.ShootingProblem(self.defaultState,[runningModel] * T,runningModel)
        ...
        us0 = [ddp.problem.runningModels[0].quasiStatic(ddp.problem.runningDatas[0],
                                                        self.defaultState)] * T
	...
	# Initialize MPC with current state of simulation
        ...
        if (self.Tswitch2single >= 0 and self.Tswitch2single <= T):
            if self.swingRightPhaseSimple:
                self.ddp.problem.updateModel(self.Tswitch2single,self.runningModelRightSwing)
                if self.verbose: print("update to swing right contact model ",self.Tswitch2single)        
            else:
                self.ddp.problem.updateModel(self.Tswitch2single,self.runningModelLeftSwing)
                if self.verbose: print("update to swing left contact model ",self.Tswitch2single)
        if (self.Tswitch2double >= 0 and self.Tswitch2double <= T):
            self.ddp.problem.updateModel(self.Tswitch2double,self.runningModel)
            if self.verbose: print("update to double contact model ",self.Tswitch2double)
        if (self.Tswitch2double >= 0 and self.Tswitch2double <= T1contact):
            if self.swingRightPhaseDouble:
                ...
                targetFrameNow.translation[0] = self.startingPosRightFoot.translation[0] + self.xForward * float(T1contact - self.Tswitch2double) / float(T1contact)
                targetFrameNow.translation[1] = self.startingPosRightFoot.translation[1] - yCorrection * float(T1contact - self.Tswitch2double) / float(T1contact)
                targetFrameNow.translation[2] = self.startingPosRightFoot.translation[2] + foot_height * np.sin(float(T1contact - self.Tswitch2double) / float(T1contact) * TFootDepth * 3.14 / 180)
                self.residualPlacementRight.reference = targetFrameNow
                if self.verbose: print('Change placement right')
            else:
                # Update left foot desired placement
                targetFrameNow = self.startingPosLeftFoot.copy()
                targetFrameNow.translation[0] = self.startingPosLeftFoot.translation[0] + self.xForward * float(T1contact - self.Tswitch2double) / float(T1contact)
                targetFrameNow.translation[1] = self.startingPosLeftFoot.translation[1] + yCorrection * float(T1contact - self.Tswitch2double) / float(T1contact)
                targetFrameNow.translation[2] = self.startingPosLeftFoot.translation[2] + foot_height * np.sin(float(T1contact - self.Tswitch2double) / float(T1contact) * TFootDepth * 3.14 / 180)
                self.residualPlacementLeft.reference = targetFrameNow
                if self.verbose: print('Change placement left')
        ...
        if (self.Tswitch2single < 0):
            ...
            if self.verbose: print('Next swight is right? ',self.swingRightPhaseSimple)
        if (self.Tswitch2double < 0):
            ...
            if self.verbose: print('switch to double: Update wrench cone orientation')
            ...
            wrenchConeFrameLeft = croc.WrenchCone(self.coneRotationLeft, mu, cone_box,4,True,minNforce,maxNforce)
            wrenchConeFrameRight = croc.WrenchCone(self.coneRotationRight, mu, cone_box,4,True,minNforce,maxNforce)
...
if __name__ == "__main__":
    ...
    rmodel, [gmodel] = pin.buildReducedModel(robot.model, [robot.visual_model ],
