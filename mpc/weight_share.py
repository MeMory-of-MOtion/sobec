...
def computeReferenceForces(contactPattern,robotweight,maxTransitionDuration=50):
    ...
    # foot at each time.
    contactImportance = weightShareSmoothProfile(contactPattern,transitionDuration,switch=switch_linear)
    # Contact reference forces are set to contactimportance*weight
    weightReaction = np.array([0,0,robotweight,0,0,0])
    referenceForces = [
        [ weightReaction*contactImportance[t,k] for k,p in enumerate(contactPattern[t]) ]
        for t in range(T) ]
