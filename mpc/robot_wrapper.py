...
def addChildrenFrames(model,parentFrameIds,subname,displacement=None,translation=None,xtranslation=None):
    '''
    ...
    '''
    assert(displacement is not None or translation is not None or xtranslation is not None)
    if displacement is None:
        if translation is None: translation = np.array([xtranslation,0,0])
        displacement = pin.SE3(np.eye(3),translation)
    for cid in parentFrameIds:
        assert(cid<len(model.frames))
        f = model.frames[cid]
        newframe = pin.Frame(f.name+"_"+subname,f.parent,f.previousFrame,
                             f.placement * displacement, pin.FrameType.OP_FRAME)
...
class RobotWrapper:
    ...
    def __init__(self,model,contactKey,refPosture="half_sitting"):
        ...
        '''
        self.model = model
        self.contactIds = [ i for i,f in enumerate(self.model.frames) if contactKey in f.name ]

        addChildrenFrames(self.model,self.contactIds,"tow",xtranslation=0.1)
        self.towIds = {idf: model.getFrameId(f"{model.frames[idf].name}_tow") for idf in self.contactIds}

        addChildrenFrames(self.model,self.contactIds,"heel",xtranslation=-0.1)
        self.heelIds = {idf: model.getFrameId(f"{model.frames[idf].name}_heel") for idf in self.contactIds}
        ...
        self.gravForce = -sum(Y.mass for Y in self.model.inertias) * self.model.gravity.linear[2]
        self.com0 = pin.centerOfMass(self.model, self.data, q0)


if __name__ == "__main__":
    
    r = RobotWrapper(talos_low.load().model,'sole_link')
    assert(len(r.towIds)==2)
