...
class WalkPlotter:
    ...
    def computeFootTraj(self):
        ...
        for x in self.xs:
            ...
            self.foottraj.append( np.concatenate([  self.data.oMf[cid].translation for cid in self.contactIds ]))
            self.footvtraj.append( np.concatenate([  pin.getFrameVelocity(self.model,self.data,cid).vector for cid in self.contactIds ]))
    ...
    def plotTimeCop(self):
        ...
        for ifig,cid in enumerate(self.contactIds):
            ...
            ftraj =[[t,f[6*ifig:6*ifig+6]] for t,(f,p) in enumerate(zip(self.fs,self.contactPattern)) if p[ifig] ]
            cop = [ [t,  [ f[4]/f[2], -f[3]/f[2] ] ] for (t,f) in ftraj ]
            plt.plot([ t for t,p in cop ], [ p for t,p in cop ],'.')

    def plotCopAndFeet(self,FOOT_SIZE,ARENA_SIZE=.6):
        ...
        for ifig,cid in enumerate(self.contactIds):
            ...
            for t,pattern in enumerate(self.contactPattern[:-1]):
                ...
                if not self.contactPattern[t][ifig]: continue
                f = self.fs[t][6*ifig:6*ifig+6]
                l_cop =  np.array([ f[4]/f[2], -f[3]/f[2],0 ])
                pin.framesForwardKinematics(self.model,self.data,self.xs[t][:self.model.nq])
    ...
    def plotCom(self,com0):
        ...
        vcomplot = []
        for x in self.xs:
            pin.centerOfMass(self.model,self.data,x[:self.model.nq],x[self.model.nq:])
    ...
    def plotFeet(self):
        ...
        hplot = []
        for i,cid in enumerate(self.contactIds):
            hplot.extend(plt.plot(np.sqrt(np.sum(self.footvtraj[:,6*i:6*i+2]**2,1))))
        plt.legend(hplot,names)
        plt.ylabel('horz vel')
