...
class SimuProxy:
    ...
    def loadPinocchio(self,model_rootpath,urdf_subpath,urdf_filename,srdf_subpath,srdf_filename):
        ...
        self.rmodel, self.gmodel_col, self.gmodel_vis = pin.buildModelsFromUrdf(urdf_path,model_rootpath,
    ...
    def loadBulletModel(self,guiOpt = pyb.DIRECT):
        ...
        self.planeId = planeId = pyb.loadURDF("plane.urdf")
        
        pyb.setAdditionalSearchPath(os.path.join(self.model_rootpath,self.urdf_subpath))
        self.robotId = robotId = pyb.loadURDF(self.urdf_filename,
                                              list(self.rmodel.q0[:3]),list(self.rmodel.q0[3:7]),
                                              useFixedBase = False)
    ...
    def setBulletFinalizationTrics(self):
        self.bullet_names2indices = { pyb.getJointInfo(1,i)[1].decode(): i for i in range(pyb.getNumJoints(1)) }
        self.bulletCtrlJointsInPinOrder = [ self.bullet_names2indices[n] for n in self.rmodel.names[2:] ]
    ...
    def changeFriction(self,names,lateralFriction=100,spinningFriction=30):
        for n in names:
            ...
            pyb.changeDynamics(self.robotId,idx,lateralFriction = lateralFriction, spinningFriction = spinningFriction)
    ...
    def freeze(self,jointNames):
        ...
        self.rmodel,[self.gmodel_col,self.gmodel_vis] \
            = pin.buildReducedModel(self.rmodel_full,
                                    [self.gmodel_col,self.gmodel_vis],
                                    jointIds,self.rmodel.q0)
    ...
    def step(self,torques):
        ...
        pyb.setJointMotorControlArray(self.robotId, self.bulletCtrlJointsInPinOrder,
                                      controlMode=pyb.TORQUE_CONTROL, forces=torques)
        pyb.stepSimulation()
